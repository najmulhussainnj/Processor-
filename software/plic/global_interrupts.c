...
void mei_isr(){
  printf("Came to local Interrupt Service Routine\n");
  plic_source int_num  = PLIC_claim_interrupt(&g_plic);
  if ((int_num >=1 ) && (int_num < PLIC_NUM_INTERRUPTS)) {
    printf("Going to Interrupt Service Routine for the Bit :%d\n",int_num);
    g_ext_interrupt_handlers[int_num]();
  }
  else {
    printf("Somehow coming to this random section\n");
...
void trap_27_handler() {
	printf(" Interrupt at 27th pin serviced");
    tohost_exit(1337);
}

void trap_28_handler() {
	printf(" Interrupt at 28th pin serviced");
}

void trap_29_handler() {
	printf(" Interrupt at 29th pin serviced");
}

void trap_30_handler() {
	printf(" Interrupt at 30th pin serviced");
    tohost_exit(1337);
}
void trap_31_handler(){
    printf(" Interrupt at 31st pin serviced");
}
//void pwm_0_handler() {
//
//
...
int main(int argc, char **argv)
{
  ...
  g_ext_interrupt_handlers[27] = trap_27_handler;
  g_ext_interrupt_handlers[28] = trap_28_handler;
  g_ext_interrupt_handlers[29] = trap_29_handler;
  g_ext_interrupt_handlers[30] = trap_30_handler;
  g_ext_interrupt_handlers[31] = trap_31_handler;
  ...
  PLIC_enable_interrupt (&g_plic, 27);
  PLIC_enable_interrupt (&g_plic, 28);
  PLIC_enable_interrupt (&g_plic, 29);
  PLIC_enable_interrupt (&g_plic, 30);
  PLIC_enable_interrupt (&g_plic, 31);
...
//  PLIC_set_priority(&g_plic, PWM0_INT_BASE + 0   , 5);
  ...
  // would win.
  PLIC_set_priority(&g_plic, 27, 32);
  PLIC_set_priority(&g_plic, 28, 16);
  PLIC_set_priority(&g_plic, 29, 8);
  PLIC_set_priority(&g_plic, 30, 4);
  PLIC_set_priority(&g_plic, 31, 2);
  PLIC_set_threshold(&g_plic, 4);
