...
#if ($test_suite =~ /riscv-torture/) {
#  systemFileCmd("cut -c1-8 $shaktiHome/config_string64.hex","boot.MSB");
#  systemFileCmd("cut -c9-16 $shaktiHome/config_string64.hex","boot.LSB");
#}
systemCmd("ln -s $shaktiHome/bin/out       out");
systemCmd("ln -s $shaktiHome/boot.MSB    boot.MSB");
systemCmd("ln -s $shaktiHome/boot.LSB    boot.LSB");
